...
class RecoverNet(nn.Module):
	def __init__(self, mode='maxpool'):
		...
		if 'maxpool' in self.mode:	# 使用最大池化下采样,例程中没用
			...
			if not 'model-wise' in self.mode:
				...
				self.downsample = self.index_block(None, use_nonlinear=True, use_context=True, use_squeeze=True,
	...
	def forward(self, input):
		...
		elif 'bilinear' in self.mode or 'combine' in self.mode:
		...
		elif 'carafe' in self.mode or 'deconv' in self.mode:
			l = self.upsample1(l)
		elif 'spacedepth' in self.mode:
